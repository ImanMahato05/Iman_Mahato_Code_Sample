...
class S_T_NETWORK():
    def __init__(self, network_name, sample_size, dis_size, input_size, hidden_layer_count, hidden_layer_neurals,
                 hidden_activate_fun, keep_prop, batch_norm, bn_is_training, output_layer_size=1, output_acitvate_fun=tf.identity,
                 weight_init='std', dist_data=None, diff_weight=False, no_network=False):
        ...
        if dist_data is None:
            self.dist_data = tf.placeholder(tf.float32, [None, self.dis_size, self.input_size])
        else:
                ...
                if not diff_weight:
                    ...
                    if hidden_layer_count > 0:
                        for layer_index in range(hidden_layer_count):
                            ...
                            hidden_layer = utils.fully_connected(
                                self.network_name + '_network_hidden_layer_' + str(layer_index),
                                input_layer,
                    ...
                    # Since the spatiotemporal models are not synchronized, and training is relatively slow, at most only one hidden layer is considered.
                    ...
                    hidden_layer = utils.fully_connected(self.network_name + '_network_hidden_layer', cur_x_data,
                                                         hidden_W,
...
class GTW_NETWORK():
    def __init__(self, network_name, st_network_input, hidden_layer_count, hidden_layer_neurals, output_size,
                 activate_fun, keep_prop, batch_norm, bn_is_training, weight_init='std'):
        self.network_name = network_name
...
class BASE_NETWORK():
    def __init__(self, network_name, network_input, hidden_layer_count, hidden_layer_neurals, output_size,
                 activate_fun=tf.identity, keep_prop=1.0, batch_norm=False, bn_is_training=False, weight_init='std'):
        self.network_name = network_name
            ...
            if hidden_layer_count > 0:
                for layer_index in range(hidden_layer_count):
                    ...
                    if weight_init_std:
                        ...
                        hidden_W = utils.init_weight_he('hidden_W_' + str(layer_index),
                                                        [input_size, hidden_layer_neurals[layer_index]], input_size,
                                                        factor)

                    hidden_b = utils.init_bias('hidden_b_' + str(layer_index), [hidden_layer_neurals[layer_index]])

                    hidden_layer = utils.fully_connected(network_name + '_hidden_layer_' + str(layer_index),
                                                         input_layer, hidden_W,
                                                         hidden_b, batch_norm, bn_is_training)
                    if self.activate_fun == "prelu":
                        hidden_layer = utils.para_relu(hidden_layer, "prelu" + str(layer_index))
                    else:
...
class DIAGNOSIS():
    def __init__(self, x_data, y_data, yhat, linear_beta_mat, gtweight, miny, maxy):
        ...
        with tf.variable_scope('DIAGNOSIS'):
            ...
            self.y_data_convert = y_data * tf.constant((maxy - miny), dtype=tf.float32) + tf.constant(miny,
                                                                                                      dtype=tf.float32)
            self.yhat_rg_convert = self.yhat_rg * tf.constant((maxy - miny), dtype=tf.float32) + tf.constant(miny,
                                                                                                             dtype=tf.float32)
            ...
            hatS_temp = tf.matmul(gtweight_3d,
                                  tf.matmul(tf.matrix_inverse(tf.matmul(x_data_tile_t, x_data_tile)), x_data_tile_t))
            ...
            # Statistical test measures (F1, F2, F3, etc.)
            I = tf.matrix_diag(tf.ones(shape=[n_size]))
            # OLS hat matrix, etc.
            hat_OLS = tf.matmul(x_data,
                                tf.matmul(tf.matrix_inverse(tf.matmul(tf.transpose(x_data), x_data)),
                                          tf.transpose(x_data)))
            ...
            # F2 test
            self.f2 = tf.squeeze(
                tf.divide(tf.divide((RSS_OLS - RSS1), trace_OLS - traceR1), tf.divide(RSS_OLS, trace_OLS)))

            # F3 test
            ek_zeros = np.zeros([x_data.shape[1]])
            ek_dict = {}
            self.f3_dict = {}
            self.f3_dict_2 = {}
            ...
            self.yhat_convert = self.yhat * tf.constant((maxy - miny), dtype=tf.float32) + tf.constant(miny,
                                                                                                       dtype=tf.float32)
            ...
            # Adjusted R2
            self.r2_adjusted_coeff = utils.coeff_r2_adjusted_tensor_cal(tf.squeeze(y_data), self.yhat, traceR1,
